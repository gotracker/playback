...
type RetrigVolumeSlide[TPeriod period.Period] DataEffect // 'R'

// Start triggers on the first tick, but before the Tick() function is called
func (e RetrigVolumeSlide[TPeriod]) Start(cs playback.Channel[TPeriod, Memory], p playback.Playback) error {
	cs.ResetRetriggerCount()
	return nil
}

// Tick is called on every tick
func (e RetrigVolumeSlide[TPeriod]) Tick(cs playback.Channel[TPeriod, Memory], p playback.Playback, currentTick int) error {
	x := DataEffect(e) >> 4
	y := DataEffect(e) & 0x0F
	if y == 0 {
		return nil
