...
func (e FineVolumeSlideUp) Tick(cs playback.Channel[channel.Memory], p playback.Playback, currentTick int) error {
	mem := cs.GetMemory()
...
// Start triggers on the first tick, but before the Tick() function is called
func (e VolChanFineVolumeSlideUp) Start(cs playback.Channel[channel.Memory], p playback.Playback) error {
	mem := cs.GetMemory()
